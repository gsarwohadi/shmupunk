...
class Command
{
	...
	public static function read(string:String) 
	{
		...
		for (line in lines) 
		{
			...
			if (commandName.charAt(0) == ':') 
			{
				...
				switch (actorTypeName)
				{
					...
					case "stage": 
						currentActorType = Stage;
					case "enemy": 
						currentActorType = Enemy;
					case "turret": 
						currentActorType = Turret;
					case "bullet": 
						currentActorType = Bullet;
					default:
		...
		if (parents.length > 0)
			addRootCommand(parents[0].name, parents[0]);
		
		if (errorCount > 0) 
			throw errorCount + " error(s)\n" + errorMessages;
	}
	
	private static inline function COMMAND_EREG()
	{
		return ~/^([@\$:]?\w+)(?:\s*)(<?[\+\-]?=>?)?(?:\s+)?(.*)$/;
	}
	...
	{
		//HXP.console.log(["Command new.", actorType, commandName, lineCount]);
		...
		if (commandOperator != null && commandOperator.length > 0) 
		{
			...
			if (!ereg.match(commandName)) 
				throw "invalid VARIABLE: " + commandName;
		...
		else
		{
			...
			if (!ereg.match(commandName))
				throw "invalid COMMAND_NAME: " + commandName;
